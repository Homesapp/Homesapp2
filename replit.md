...
*   **External Property Management System**: Multi-tenant module for managing external agency properties independently, with payment calendar, maintenance tickets, and optional property linking to main system. Includes granular role-based permissions, agency logo upload, user reassignment, backend validation, and a condominium-unit management system (hierarchical property structure for multi-unit condominiums and standalone units).
    ...
    *   **Maintenance Worker Assignment System**: Complete workflow for assigning specialized maintenance workers to specific condominiums or units. Worker assignments reference the user (userId) and location (condominiumId or unitId) without duplicating specialty data. Features dedicated ExternalMaintenanceWorkers page with tabbed interface displaying worker specialty from user records, assignment CRUD operations, and multi-tenant security enforcement. Backend routes with ownership verification prevent cross-agency worker assignments. Multi-select assignment capability allows assigning a single worker to multiple condominiums, multiple units, or all units within selected condominiums simultaneously via checkbox-based UI. Batch assignment endpoint `/api/external-worker-assignments/batch` with comprehensive multi-tenant validation ensures secure mass-assignment operations. Assignments are grouped by worker in the UI, showing all locations (condominiums/units) as badges for each worker. Edit functionality allows modifying worker assignments by deleting old ones and creating new ones atomically, with duplicate ID prevention to avoid race conditions.
    ...
    *   **Enhanced Maintenance Form with Calendar UI**: Redesigned maintenance ticket creation form with always-visible calendar picker (no hidden icons), Cancun timezone (America/Cancun) support for all date/time operations, two-step unit selection (condominium first, then unit), and intelligent worker filtering (only shows workers assigned to selected unit/condominium via externalWorkerAssignments). Form includes visual datetime confirmation and larger responsive modal dialog.
    *   **Temporary Credentials Management for External Users**: Complete workflow for sharing user credentials after account creation. Features include: automatic temporary password generation with `requirePasswordChange` flag set to true, UI card displaying both email and password with copy-to-clipboard buttons, forced password change on first login via ForcePasswordChange component, auto-verification of external agency user emails, secure credential display with dismissible card after user creation.
    *   **Unit Information Quick-Share System**: Enhanced ExternalUnitDetail page with one-click copy-to-clipboard functionality for unit information (condominium, unit number, bedrooms, bathrooms, area, Airbnb photos link) and access controls (WiFi, door codes, gates, etc.). Includes visual feedback with temporary check icons and toast notifications. Added `airbnbPhotosLink` field to externalUnits table for storing Airbnb photo gallery URLs.
    *   **Advanced Access Control Filtering & Bulk Actions**: Redesigned ExternalAccesses page with cascading dropdown filters (condominium → unit selection, access type filter, search), multi-select functionality using checkboxes, and bulk copy action for sharing multiple access codes simultaneously. Selection state automatically clears when filters change to maintain accurate count displays. Filter panel includes real-time counter showing number of selected accesses with quick copy button.
