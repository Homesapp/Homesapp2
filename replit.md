...
All External sections (Accounting, Accesses, Condominiums, Maintenance) implement a unified filter UX using Popover modals. Each section features a compact filter icon button that opens a dropdown Popover (width: 24rem, max-height: 600px with scroll, right-aligned) containing all filter options with toggle buttons. This design provides a cleaner, more space-efficient interface while maintaining full filtering capabilities. Each Popover includes a "Clear Filters" button at the bottom for easy reset.
