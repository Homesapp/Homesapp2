...
*   **Calendly-Style Appointment Scheduling (Oct 2025)**: Completely redesigned the appointment scheduling dialog with a modern, professional 4-step wizard interface similar to Calendly. Features include: step-by-step progress indicators, large visual cards for appointment type selection, centered calendar display for date selection, grid-based time slot selection (replacing dropdown), and comprehensive confirmation summary with icons. Backend enhanced to properly handle JSON date serialization. Significantly improved user experience with clearer navigation and better visual hierarchy.
*   **Appointment System Enhancements (Oct 2025)**: Fixed auto-submit bug by preventing Enter key from submitting form (now blocks Enter on all fields EXCEPT textareas to preserve multi-line notes). Form only submits when user explicitly clicks "Confirmar cita" button. Implemented business rule: clients limited to 1 appointment per day with backend validation that prevents booking multiple appointments for same date. Enhanced role-based permissions: clients can only cancel pending appointments, NOT confirm them (only admin/master/admin_jr/owner/seller can confirm). Refactored client Appointments page with professional calendar view (weekly navigation, status filtering, appointment details dialog) matching owner UX parity.
