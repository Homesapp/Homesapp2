...
*   **Appointment System Enhancements (Oct 2025)**: Fixed auto-submit bug by preventing Enter key from submitting form in steps 1-3 of appointment dialog (only allows submit on final confirmation step). Implemented business rule: clients limited to 1 appointment per day with backend validation that prevents booking multiple appointments for same date. Enhanced role-based permissions: clients can only cancel pending appointments, NOT confirm them (only admin/master/admin_jr/owner/seller can confirm). Refactored client Appointments page with professional calendar view (weekly navigation, status filtering, appointment details dialog) matching owner UX parity.
