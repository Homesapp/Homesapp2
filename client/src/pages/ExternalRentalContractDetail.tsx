...
export default function ExternalRentalContractDetail() {
  ...
  const handleSubmitSchedule = async (data: ScheduleFormData) => {
    // Ensure contractId is set (guard against empty string from initial render)
    ...
    if (editingSchedule) {
      ...
      // Explicitly set contractId from params
    ...
    <div className="container mx-auto px-4 py-6 space-y-4">
      ...
      <div className="flex items-center gap-3">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => navigate('/external-management')}
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="text-sm" data-testid="text-start-date">
              {new Date(contract.startDate).toLocaleDateString(language === "es" ? "es-MX" : "en-US")}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">{language === "es" ? "Fin" : "End Date"}</p>
            <p className="text-sm" data-testid="text-end-date">
              {new Date(contract.endDate).toLocaleDateString(language === "es" ? "es-MX" : "en-US")}
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-2">
          ...
          <Button
            size="sm"
            onClick={() => {
              // Only open dialog if contract is loaded and id is available
                ...
                >
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground" data-testid={`text-payment-due-${payment.id}`}>
                      {language === "es" ? "Vence:" : "Due:"} {new Date(payment.dueDate).toLocaleDateString(language === "es" ? "es-MX" : "en-US")}
                    </p>
                  </div>
                </div>
              ))}
                    ...
                    <Select onValueChange={field.onChange} value={field.value}>
                      ...
                      <SelectContent>
                        {Object.entries(serviceTypeTranslations)
                          .filter(([key]) => key !== 'rent') // Exclude rent as it's auto-created
